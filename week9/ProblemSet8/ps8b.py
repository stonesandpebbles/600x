...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    totNumViruses = [0 for x in range(numSteps + 150)]
    totResViruses = [0 for x in range(numSteps + 150)]
    totNumVirusesT = []
    for j in range(numTrials):
        viruses = []
        for i in range(numViruses):
        ...
        #totNumViruses = [0 for x in range(300)]
        #totNumViruses[0] += patient.update()
        for k in range(numSteps + 150):
            if k == numSteps:
                patient.addPrescription('guttagonol')
            totNumViruses[k] += patient.update()
            #totResViruses[k] += patient.getResistPop(['guttagonol'])
        totNumVirusesT.append(totNumViruses[-1])
    """        
    #avgNumViruses = [totNumViruses[y]/float(numTrials) for y in range(300)]   
...
simulationWithDrug(1, 20, 1.0, 0.0, {"guttagonol": True}, 1.0, 5)
    """
    pylab.hist(totNumViruseT, bins = 10)
    pylab.title('Histogram for trials')
    pylab.xlabel('Population')
    pylab.ylabel('Trials')
    pylab.xlim(min(totNumVirusesT), max(totNumberVirusesT))
