...
from ps8b import * #@UnusedWildImport
...
class CommonTests(object):
    ...
    def test_3(unit, virus, patient):
        ...
        try:
            ...
            msg = ('The reproduce() method must always return a ' + 
                   virus.__class__.__name__ + ' object, in ' + 
                   'other words, it must always reproduce, ' + 
                   'when maxBirthProb is 1.0 and popDensity is 0.0.')
            unit.assertTrue(False, msg)
        try:
            args[0] = 1.0
            for _ in range(1024):
                virus.reproduce(*args)
            msg = ('The reproduce() method must never return a ' + 
                   virus.__class__.__name__ + ' object, in ' + 
                   'other words, it must never reproduce, ' + 
                   'when popDensity is 1.0.')
            unit.assertTrue(False, msg)
        except NoChildException:
            pass
        patient.getViruses()
        ...
        # If a single random() call wasn't made, we make it here, 
        # to trigger the code on the custom random method
        random.random()
