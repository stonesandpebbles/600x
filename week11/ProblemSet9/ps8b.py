...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if reproduce:
            ...
            offresistances = self.resistances.copy()
            if choice < repProb:
                for i in self.resistances.keys():
                    if random.random() < self.mutProb:
                        if offresistances[i]:
                            offresistances[i] = False
                        else:
                            offresistances[i] = True
                            
               
                return ResistantVirus(self.maxBirthProb, self.clearProb, offresistances, self.mutProb)
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for j in self.viruses:
            try:
                offsp = j.reproduce(popDensity, self.activeDrugs)
                offspring.append(offsp)
            except NoChildException:
                pass
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
                       mutProb, numTrials, numSteps1, numSteps2):
    """
    ...
    #totResViruses = [0 for x in range(numSteps1 + numSteps2 + 150)]
    totNumVirusesT = []
    for j in range(numTrials):
        total = 0
        viruses = []
        ...
        for k in range(numSteps1 + numSteps2 + 150):
            if k == numSteps1:
                patient.addPrescription('guttagonol')
            if k == numSteps1+ numSteps2:
                patient.addPrescription('grimpex')
            total = patient.update()
            #totNumViruses[k] += patient.update()
            #totResViruses[k] += patient.getResistPop(['guttagonol'])
        totNumVirusesT.append(total)
    return totNumVirusesT
    """        
    #avgNumViruses = [totNumViruses[y]/float(numTrials) for y in range(300)]   
...
simulationWithDrug(1, 20, 1.0, 0.0, {"guttagonol": True}, 1.0, 5)
    ...
    pylab.hist(totNumVirusesT, bins = 50)
    pylab.title('Histogram for trials')
    pylab.xlabel('Population')
    pylab.ylabel('Trials')
    pylab.xlim(min(totNumVirusesT), max(totNumVirusesT))
    pylab.show()
    """
